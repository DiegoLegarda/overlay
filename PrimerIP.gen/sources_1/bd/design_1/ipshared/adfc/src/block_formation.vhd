...
architecture Behavioral of hog_block_histogram is
    ...
    signal step_counter           : integer range 0 to 6 := 0; -- Contador de pasos (aumentado)
    signal start_binarization     : STD_LOGIC := '0'; -- Nueva señal para iniciar la binarización

    -- Nuevas señales para registrar la entrada
...
begin
    -- Registro de la entrada block_histogram_in y block_valid_in
    P_INPUT_REGISTER: process (clk, reset)
    begin
        if rising_edge(clk) then
            if reset = '1' then
                ...
                r_block_valid_in <= '0'; -- Mantenemos la validez solo cuando la entrada es válida
            end if;
        end if;
    ...
    port map (
        ...
        average             => s_average,
        block_histogram_in  => r_block_histogram_in, -- Usamos la señal registrada
    ...
    GEN_SUM_STAGE_1: for i in 0 to 17 generate
        sum_stage_1(i) <= resize(unsigned(r_block_histogram_in(i / NUM_BINS)(i mod NUM_BINS)), BIN_WIDTH + 9) +
                            resize(unsigned(r_block_histogram_in((i + 18) / NUM_BINS)((i + 18) mod NUM_BINS)), BIN_WIDTH + 9);
    ...
    begin
        if rising_edge(clk) then
            ...
            elsif processing = '1' and step_counter = 3 then -- Ajustamos el paso
    ...
    begin
        if rising_edge(clk) then
            if reset = '1' then
                s_average <= (others => '0');
                s_average_ready <= '0';
            elsif processing = '1' and step_counter = 4 then -- Ajustamos el paso
    ...
    begin
        if rising_edge(clk) then
            if reset = '1' then
            ...
            elsif r_block_valid_in = '1' and processing = '0' then -- Usamos la señal registrada
                processing <= '1';
                step_counter <= 1;
                ...
                case step_counter is
                    when 1 =>
                        step_counter <= 2; -- Esperar el registro de la entrada
                    when 2 =>
                        step_counter <= 3; -- Iniciar el cálculo de la suma
                    when 3 =>
                        step_counter <= 4; -- Esperar un ciclo para que la suma se complete
                    when 4 =>
                        step_counter <= 5; -- Calcular el promedio
                    when 5 =>
