...
architecture Behavioral of top_hog_axi is
    ...
    signal hist_binarized_buffer : STD_LOGIC_VECTOR(35 downto 0) := (others => '0');
     ...
     -- Declaración del tipo de estados con todos los estados necesarios.
            ...
            signal current_state_fsm : std_logic_vector(3 downto 0);
            
            -- Señales intermedias para almacenar índices parciales (pueden ser registros o señales internas)
            signal sceldas_X_var : integer range 0 to CELLS_PER_ROW+1;
            signal sceldas_Y_var : integer range 0 to CELLS_PER_ROW+1;
            signal reg_stop_left     : integer ;
            signal reg_stop_right    : integer ;
            signal reg_sbottom_left  : integer ;
            signal reg_sbottom_right : integer ;
...
--            signal last_s_Contador : integer range 0 to IMAGE_WIDTH*IMAGE_WIDTH;
--            signal tmp_stop_left     : integer range 0 to CELLS_PER_ROW-1;
--            signal tmp_stop_right    : integer range 0 to CELLS_PER_ROW-1; 
--            signal tmp_sbottom_left  : integer range 0 to CELLS_PER_ROW-1;
--            signal tmp_sbottom_right : integer range 0 to CELLS_PER_ROW-1;
...
out_top_left<=std_logic_vector(to_unsigned(reg_stop_left, 4));
out_top_right<=std_logic_vector(to_unsigned(reg_stop_right, 4));
out_bottom_left<=std_logic_vector(to_unsigned(reg_sbottom_left, 4));
out_bottom_right<=std_logic_vector(to_unsigned(reg_sbottom_right, 4));
out_cell_x_counter<=std_logic_vector(to_unsigned(sceldas_X_var, 3));
out_cell_y_counter<=std_logic_vector(to_unsigned(sceldas_Y_var, 3));
out_celda<=s_celda;
out_FSM<=current_state_fsm;
out_Histograma<=hist_serialized_data;

     salidaFSM:  process(clk)
            begin
                if rising_edge(clk) then
                    case current_state is
                        when IDLE                => current_state_fsm <= "0000";
                        when WAIT_FOR_HISTO      => current_state_fsm <= "0001";
                        when CALC_TOP            => current_state_fsm <= "0010";
                        when CALC_BOTTOM         => current_state_fsm <= "0011";
                        when READ_BINS_A         => current_state_fsm <= "0100";
                        when READ_BINS_B         => current_state_fsm <= "0101";
                        when GENERATE_BLOCK      => current_state_fsm <= "0110";
                        when FINAL_BLOCK_CALC    => current_state_fsm <= "0111";
                        when FINAL_BLOCK_READ_A  => current_state_fsm <= "1000";
                        when FINAL_BLOCK_READ_B  => current_state_fsm <= "1001";
                        when FINAL_BLOCK_OUTPUT  => current_state_fsm <= "1010";
                        when others              => current_state_fsm <= "1111"; -- error/debug
                    end case;
                end if;
            end process;
...
Control_Bloques:process(clk, reset)
    -- Variables locales para cálculos aritméticos.
    variable tmp_stop_left     : integer ;
    variable tmp_stop_right    : integer ;
    variable tmp_sbottom_left  : integer ;
    variable tmp_sbottom_right : integer;
                    ...
                    if s_contador_reg > MAX_PIXELS - (TOTAL_BLOCKS - 1) * 10 then
                        if (s_contador_reg mod 10) = 0 then
                            if last_s_Contador /= s_contador_reg then
                                ...
                                if sceldas_X_var >= CELLS_PER_ROW-1 then
                                    sceldas_X_var <= 1;
...
--                tmp_sbottom_left  := reg_stop_right + (3);
--                tmp_sbottom_left  := 6;
...
--                if s_lock_ultimo_bloque = '0' then
                    sceldas_X_var <= CELLS_PER_ROW-1;
                    sceldas_Y_var<= CELLS_PER_ROW-1;
--                end if;
--                tmp_stop_left  := (sceldas_Y_var - 1) * CELLS_PER_ROW + sceldas_X_var;
                tmp_stop_left  := (sceldas_Y_var - 1) * 4 + sceldas_X_var;
                tmp_stop_right := tmp_stop_left + 1;
                reg_stop_left  <= tmp_stop_left;
                reg_stop_right <= tmp_stop_right;
...
--    m_axis_tvalid <= block_valid;
    m_axis_tlast<=output_last_int and s_lock_ultimo_bloque;
